...
var number_value = document.getElementsByClassName("personNumber__select")[0];
    ...
    function changeSlide2() {
        var ra = (Number(number_value.value)-4)/(18-4);
    ...
    function reducePerson() {
        if(Number(number_value.value) == 4) {
            alert("小不了");
        }else {
            number_value.value--;
            number_show.value = number_value.value;
    ...
    temp_li = list.getElementsByTagName("li");
    function createList() {
        var haha = [];
        while(temp_li.length > 0){
            removeElement(temp_li[0]);
        }
        if (number_value.value <= 4){
            ...
            var special1 = shuffle(Number(number_value.value),1);
            for(var r=0; r<1; r++) {
                ...
                list.replaceChild(li_special,list.childNodes[special1[r]]);
                haha.push(special1[r]);
            }
        }   else if(number_value.value > 4 && number_value.value <= 8) {
            ...
            for(var j=0; j<2; j++) {
                ...
                haha.push(special2[j]);
            }
        }   else if(number_value.value > 8 && number_value.value <= 12) {
            for(var y=0; y<number_value.value; y++) {
                var li = document.createElement("li");
                var li_text = document.createTextNode("水民");
                li.setAttribute("class","distribution__item");
                li.appendChild(li_text);
            ...
            var special2 = shuffle(Number(number_value.value),3);
            for(var u=0; u<3; u++) {
                var li_special = document.createElement("li");
                var li_textspecical = document.createTextNode("卧底");
                li_special.setAttribute("class","distribution__item--special distribution__item");
                li_special.appendChild(li_textspecical);
                list.replaceChild(li_special,list.childNodes[special2[u]]);
                haha.push(special2[u]);
            }
        }   else if(number_value.value > 12 && number_value.value <= 16) {
            for(var z=0; z<number_value.value; z++) {
                var li = document.createElement("li");
                var li_text = document.createTextNode("水民");
                li.setAttribute("class","distribution__item");
                li.appendChild(li_text);
            ...
            var special2 = shuffle(Number(number_value.value),4);
            for(var x=0; x<4; x++) {
                var li_special = document.createElement("li");
                var li_textspecical = document.createTextNode("卧底");
                li_special.setAttribute("class","distribution__item--special distribution__item");
                li_special.appendChild(li_textspecical);
                list.replaceChild(li_special,list.childNodes[special2[x]]);
                haha.push(special2[x]);
            }
        }
        else {
            ...
            var special3 = shuffle(Number(number_value.value),5);
            for(var m=0; m<5; m++) {
                var li_special = document.createElement("li");
                var li_textspecical = document.createTextNode("卧底");
                li_special.setAttribute("class","distribution__item--special distribution__item");
    ...
    function fa() {
        ...
        if(number_show.value < 4 || number_show.value > 18){
            alert("请输入6-18");
        }
    }
    var EventUtill = {
    ...
    function pageArray(){
        ...
        result.push(String(temp_li.length));
    ...
    distribution_btn.addEventListener("click",pageArray,false);
